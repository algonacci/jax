...
def _transpose_pads_to_padtype(kernel_sdims, strides, padding, is_conv1d):
  ...
  for k, s, (begin, end) in zip(kernel_sdims, strides[1:-1], padding):
    ...
    if is_conv1d:
      # Only check along the primary dimension.
      break
...
def _normalize_window_strides(window_strides, is_conv1d):
  ...
  if is_conv1d:
    # For a conv1d, the window_strides will be either an int or a length 1 list.
    # Insert stride 1 for the dummy dimension, then use the usual 2d logic.
  ...
  *rhs_spatial_shapes, _, rhs_out_channel = rhs.shape
  ...
  if is_transpose:
    padding_type = _transpose_pads_to_padtype(
        rhs_spatial_shapes, lhs_dilation, padding, is_conv1d)
  else:
